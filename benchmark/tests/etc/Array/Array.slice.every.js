...
[10000, 1000, 100, 50, 10, 3].forEach((arraySize) => {
    globalThis.benchmarks.push(() => {
        return {
            ...
            subcategory: `array[${arraySize}], positive params`,
            expected: 'plugin',
            options: {
                setup: eval(`() => {
                    let res = '';
...
[10000, 1000, 100, 50, 10, 3].forEach((arraySize) => {
    globalThis.benchmarks.push(() => {
        return {
            supercategory: 'Array',
            category: `Array.slice.every vs plugin`,
            subcategory: `array[${arraySize}], negative first param`,
            expected: 'plugin',
            options: {
                setup: eval(`() => {
                    let res = '';
                    const start = ${-arraySize + 1};
                    const nums = new Array(${arraySize});
            ...
            tests: [
                {
                    title: 'Array.slice.every ',
                    fn: function () {
                        res = nums.slice(start).every((x, i) => x > 0);
                    },
                },
                {
                    title: 'plugin            ',
                    fn: function () {
                        res = sliceEveryHelper(nums, start, undefined, (x, i) => x > 0);
                    },
                },
            ],
        };
...
[10000, 1000, 100, 50, 10, 3].forEach((arraySize) => {
    globalThis.benchmarks.push(() => {
        return {
            supercategory: 'Array',
            category: `Array.slice.every vs plugin`,
            subcategory: `array[${arraySize}], negative seond param`,
            expected: 'plugin',
            options: {
                setup: eval(`() => {
                    let res = '';
