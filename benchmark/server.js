...
if (!target || !['node', 'browsers'].includes(target)) {
    throw new Error('The platform must be specified in the command parameters: "node" or "browsers"');
}

const isNodeTaget = target === 'node';
const host = '0.0.0.0';
const port = isNodeTaget ? 3001 : 3000;
const benchmarkResultsFileName = `./benchmark/${target}.json`;
...
server.get("/tests/Array's cahins of methods/:file", (req, res) => {
    const stream = fs.createReadStream(
        path.resolve(`./benchmark/tests/Array's cahins of methods/${decodeURIComponent(req.params.file)}`),
    );
    res.type('application/javascript').send(stream);
});
